...
struct OnboardingQuickEditAuth: View {
    ...
    @ObservedObject private var activationManager = LifetimeActivationManager.shared
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            if activationManager.stats == nil {
                Task { await activationManager.fetchStats() }
            }
        }
    ...
    // MARK: - Child Components: Claim Header

    private var claimHeaderView: some View {
        VStack(alignment: .center, spacing: 12) {
            if let stats = activationManager.stats, stats.claimed < stats.cap {
                Text(String.localized("You are #\(stats.claimed + 1)", table: "Onboarding"))
                    .styleText(size: 14, weight: .regular, color: Color.S_0)
            ...
            if let cap = activationManager.stats?.cap {
                Text(
                    String.localizedStringWithFormat(
                        String.localized("Available to the first %d sign-ups!", table: "Onboarding"),
                        cap
                    )
                )
                .styleText(size: 17, weight: .regular, color: Color.S_1, align: .center)
            } else {
                Text(String.localized("Available to the first 5,000 sign-ups!", table: "Onboarding"))
                    .styleText(size: 17, weight: .regular, color: Color.S_1, align: .center)
            }
        }
    }

    private var claimTitle: AttributedString {
        var result = AttributedString()

        var claim = AttributedString(String.localized("Claim your\n", table: "Onboarding"))
        claim.font = .custom("LibreBaskerville-Regular", size: 36)
        claim.foregroundColor = Color.S_0

        var free = AttributedString(String.localized("Free", table: "Onboarding"))
        free.font = .custom("LibreBaskerville-Italic", size: 36)
        free.foregroundColor = Color.S_0

        var seat = AttributedString(String.localized(" seat", table: "Onboarding"))
        seat.font = .custom("LibreBaskerville-Regular", size: 36)
        seat.foregroundColor = Color.S_0
